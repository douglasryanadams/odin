...
As of the search rebuild's Phase 2, the same `BRAVE_API_KEY` is also forwarded straight to the web service (see `compose/docker-compose.yml` / `docker-compose.prod.yml`), where the first-party `BraveBackend` consumes it directly when `BRAVE_ENABLED=true`. Until SearXNG is retired, leave `BRAVE_ENABLED=false` so only the `braveapi` engine bills Brave; once the direct client is proven, enable it and set `SEARXNG_ENABLED=false`.

### Other engines

`startpage`, `qwant`, `karmasearch`, and `wikipedia` are SearXNG defaults that we override with `disabled: true` in the engines list. The first three consistently returned CAPTCHA / access-denied from our cloud IP. `wikipedia` returns HTTP 429 because SearXNG identifies itself as `searxng/<version>` and that trips Wikimedia's bot rate limit — SearXNG has no per-engine User-Agent override, and even the global `outgoing.useragent_suffix` only appends rather than replacing the bot-shaped prefix. None of these can be dropped via `use_default_settings.engines.remove` because SearXNG's network config references some of these names as aliases (`qwant` in particular) and removal causes a `KeyError` at startup.

Wikipedia URLs still reach the synthesis pipeline whenever Brave surfaces them in SERP results — the fetch layer pulls the article, Claude cites it — so disabling the Wikipedia engine does not drop Wikipedia from the output. We just lose the engine's own "did you mean / related-articles" surface, which is low value given `claude.generate_queries` does query expansion. `TODO.md` tracks a future option to integrate Wikipedia via OAuth Personal API Token (the only way to get a higher rate limit) if we ever measure Brave missing valuable Wikipedia hits.
