...
_VALID_SECRET = "x" * 32
_DB_URL = "postgresql://odin:odin@odin-postgres:5432/odin"
...
def test_secret_key_below_32_chars_rejected() -> None:
    ...
    with pytest.raises(ValidationError, match="secret_key"):
        Settings(secret_key="too-short", app_url=_APP_URL, database_url=_DB_URL)  # noqa: S106


def test_secret_key_at_32_chars_accepted() -> None:
    """Exactly 32 chars satisfies the minimum length."""
    settings = Settings(secret_key=_VALID_SECRET, app_url=_APP_URL, database_url=_DB_URL)
...
def test_cookie_secure_can_be_disabled() -> None:
    ...
    settings = Settings(
        secret_key=_VALID_SECRET, app_url=_APP_URL, database_url=_DB_URL, cookie_secure=False
    )
