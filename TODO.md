...
1. **Harden search-result reliability (remainder).** The first slice — swapping SearXNG's scraping `brave` engine for the official `braveapi` and pruning the consistently blocked `startpage` / `qwant` scrapers — landed alongside the Brave API key provisioning. Outstanding: Python-side retries with backoff around `searxng.search` and the five Claude calls in `claude.py`, per-source timeouts in the pipeline, partial-result user-facing messaging (e.g., "profile built from 3 of 5 sources"), and per-stage failure metrics. Each is a separate small increment.
